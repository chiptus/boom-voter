...
export const VotePerspectiveSelector = ({ 
  ...
  const perspectiveOptions = [
    { id: 'all', label: 'All Groups', description: 'View ratings from all users' },
    { id: 'mine', label: 'My Votes Only', description: 'View only your personal votes' },
    ...groups.map(group => ({ 
      id: group.id, 
      label: group.name, 
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h4 className="text-sm font-medium text-purple-200">Vote Perspective</h4>
        </div>
        <p className="text-xs text-purple-400 mb-3">Choose whose votes to show in ratings</p>
        <Select value={getCurrentValue()} onValueChange={handleChange}>
          ...
          <SelectContent className="bg-gray-800 border-purple-400/30">
            {perspectiveOptions.map(option => (
              <SelectItem key={option.id} value={option.id} className="text-purple-100">
                <div>
                  <div className="font-medium">{option.label}</div>
                  <div className="text-xs text-purple-400">{option.description}</div>
                </div>
              </SelectItem>
            ))}
          </SelectContent>
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-purple-400">Choose whose votes to show in ratings</p>
      <div className="flex flex-wrap gap-2">
        <Button
          variant={!selectedGroupId ? "default" : "outline"}
          size="sm"
          onClick={() => onGroupChange(undefined)}
          className={!selectedGroupId
        ...
        {user && (
          <Button
            ...
            className={selectedGroupId === user.id
              ? "bg-purple-600 hover:bg-purple-700"
              : "border-purple-400 text-purple-400 hover:bg-purple-400 hover:text-white"
            }
          >
