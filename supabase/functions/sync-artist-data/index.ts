...
function extractSoundCloudDataWithCheerio(html: string) {
  ...
  $("script").each((_, element) => {
    ...
    if (scriptContent && scriptContent.includes("window.__sc_hydration")) {
      try {
        const hydrationMatch = scriptContent.match(
          ...
          for (const item of hydrationData) {
            if (item && item.hydratable === "user" && item.data) {
              ...
              if (userData.followers_count !== undefined) {
                ...
                console.log(
                  `Found followers count in hydration: ${followersCount}`,
                );
              }
    ...
    $('script[type="application/ld+json"]').each((_, element) => {
      try {
        ...
        if (jsonData.interactionStatistic && !followersCount) {
          const followStat = jsonData.interactionStatistic.find(
...
async function processSoundCloudArtists(
  ...
  for (const artist of artists) {
    try {
      ...
      if (
        scrapedData.followersCount !== null &&
      ...
      if (
        ...
        (!artist.image_url || scrapedData.avatarUrl.includes("t500x500"))
      ) {
      ...
      results.failed++;
      results.errors.push(
        `Processing error for ${artist.name}: ${error.message}`,
      );
