...
test("should display festival selection page", async ({ page }) => {
  ...
  await page
    .getByText(
      "Choose a festival to start voting and collaborating with your community.",
    )
    .isVisible();

  await page.getByRole("list", { name: "festival list" }).isVisible();
});

test("should display available festivals or empty list", async ({ page }) => {
  await testHelpers.navigateTo("/");

  // Look for festival cards or list items
  const list = page.getByRole("list", { name: "festival list" });
  const festivalItems = list.getByRole("listitem");
...
test("should navigate to festival editions", async ({ page }) => {
  ...
  if (await festivalItems.first().isVisible()) {
    await festivalItems.first().click();
    ...
    await expect(
      page
        .getByText("Choose Festival Edition")
        .or(page.getByText("artists available for voting"))
        .or(page.getByText("No editions available", {})),
    ).toBeVisible();
  }
});
